...
static int global_memloc = 0;
static int local_memloc = MAX_MEM - 2;

/* Nome da função atual */
static char *currentFunction = NULL;

/* Flag to see if compound is comming from function */
static bool isFromFunction = FALSE;
...
static void insertNode(TreeNode *t)
{
    ...
    case StmtK:
        ...
        case VarK:
            ...
            {
                if (strcmp(currentScope->funcName, "global") == 0)
                {
                    if (t->isArray)
                    {
                        global_memloc += t->child[0]->attr.val;
                        st_insert(t->attr.name, t->lineno, global_memloc++, t->type, VarK, t->isArray, currentScope->funcName);
                    }
                    else
                    {
                        st_insert(t->attr.name, t->lineno, global_memloc++, t->type, VarK, t->isArray, currentScope->funcName);
                    ...
                    if (t->isArray)
                    {
                        st_insert(t->attr.name, t->lineno, local_memloc--, t->type, VarK, t->isArray, currentScope->funcName);
                        local_memloc -= t->child[0]->attr.val;
                    }
                    else
                    {
                        st_insert(t->attr.name, t->lineno, local_memloc--, t->type, VarK, t->isArray, currentScope->funcName);
        ...
        case FuncK:
            ...
            local_memloc = MAX_MEM - 2;

            // Inserir função na tabela de símbolos
            if (st_lookup(t->attr.name) == NULL)
            {
                st_insert(t->attr.name, t->lineno, MAX_MEM - 1, t->type, FuncK, t->isArray, "");
        ...
        case ParamK:
            ...
            if (st_lookup_top(t->attr.name) == NULL)
            {
                if (strcmp(currentScope->funcName, "global") == 0)
                {
                    st_insert(t->attr.name, t->lineno, global_memloc++, t->type, ParamK, t->isArray, currentScope->funcName);
                }
                else
                {
                    st_insert(t->attr.name, t->lineno, local_memloc--, t->type, ParamK, t->isArray, currentScope->funcName);
                }
            }
            else
            {
        ...
        case CompoundK:
        {
            if (isFromFunction || currentScope->funcName == "global")
            {
        ...
        case ReturnK:
        {
            ...
            if (currentFunction != NULL) {
                ...
                if (symbol != NULL) {
                    ...
                    // Se a função não é void e não há expressão no return
                    if (funcType != Void && t->child[0] == NULL) {
                        pce("Semantic error at line %d: Missing return value in function returning non-void\n", t->lineno);
                        Error = TRUE;
                    }
                }
            }
